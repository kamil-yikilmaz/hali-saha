...
function passwordForm(){
 root.innerHTML='<form class="card" id="passwordForm"><h2>Şifre değiştir</h2><p>En az 14 karakterlik benzersiz bir parola kullanın.</p><label>Mevcut şifre<input id="oldPassword" type="password" autocomplete="current-password" required></label><label>Yeni şifre<input id="newPassword" type="password" autocomplete="new-password" minlength="14" maxlength="128" required></label><label>Yeni şifre tekrar<input id="againPassword" type="password" autocomplete="new-password" minlength="14" maxlength="128" required></label><button>Değiştir</button><button type="button" id="back">Geri</button></form>';
 bind('back',()=>call('read'));
 document.getElementById('passwordForm').onsubmit=e=>{e.preventDefault();run(async()=>{const password=document.getElementById('newPassword').value;if(password!==document.getElementById('againPassword').value){message('Yeni şifreler aynı olmalı.');return;}checked(await sb.auth.signInWithPassword({email:session.user.email,password:document.getElementById('oldPassword').value}));checked(await sb.auth.updateUser({password}));await logout();message('Şifre değiştirildi. Yeniden giriş yapın.');});};
}
